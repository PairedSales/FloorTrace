...
1. ~~Run `scripts/ocrBenchmark.mjs` (which already exercises the JS-CLAHE path, since Node never loads OpenCV)~~ **— this premise was false and has been resolved.** OpenCV *does* resolve in Node (271–410 ms, `cv.Mat` present), and `ocrBenchmark.mjs` runs a full warm-up scan before every measured fixture, so the bench took the **OpenCV** branch for every fixture including the first. The JS-CLAHE path is now reachable for comparison via `FLOORTRACE_NO_OPENCV=1` (read inside `loadOpenCv`, because the warm-up scan primes the singleton before any later check). Measured over all seven fixtures: OpenCV 62/82 with 2 false positives, JS-only 61/82 with 4. **OpenCV earns its branch, so it was not deleted.**
2. ~~If OpenCV measurably helps some scans, defer `loadOpenCv()`…~~ **Resolved differently.** The real defect was not *when* the load started but that `pipeline.js` peeked with `openCvIfReady()` with only synchronous work between the `import()` and the peek — which a dynamic import can never beat. The first scan of a session therefore *always* took JS CLAHE and later scans took cv.CLAHE, i.e. the same plan preprocessed two different ways depending on download latency. `detectDimensionsCore` now awaits the load to a bounded deadline (`CV_WAIT_MS`) instead of peeking.
