...
The exterior stage is a **hypothesise-and-score search**, the same shape as room detection and OCR — not a single sealing heuristic. Stages:
  ...
  - `nonGla.js` — garage/porch/patio arbitration. Four detectors (OCR label votes, label floods, geometric garage evidence, shaded pockets) emit *candidate regions*; overlapping candidates merge into one region carrying both sources; one pass removes them under a cumulative bound, so the result cannot depend on detector order. **A `shaded` carve is not a stated one and is treated differently in three places**, because a bathroom floor is tinted exactly as a condo balcony is and only the OCR pass can tell them apart: a pocket a parsed dimension label is written on is refused; the carve does not join `carvedRegions` (`pipeline.js`), so it cannot exempt the very labels it removed from `label-outside`; and its `area-excluded` warning is `warn`, not `info`, so it is counted as a thing to check. Ungoverned, all three together deleted two baths from ExampleFloorplan9 at 93% confidence with nothing said. **The label test is asked of the tint *field*, not of each component** — `groupTintFields` closes across thin ink, blocked by `thickMask` — because a label sits on a bathroom's floor and never on its bath, so per-fragment the bath is an unlabelled pocket and comes out on its own
  ...
  **The room-click clamp is a rail, not a definition of living area, and its two failure modes are not symmetric** (`roomClampBoundary`, `detectRoomFromClickCore`). Too loose and wall coverage bounds the growth instead, which is what the unclamped path already does; too tight and the click starts outside its own bound, `growRoomRect` returns null, and the user clicks a room and nothing happens. So the clamp asks for the widest hypothesis — `autoGarage: false` **and `autoShaded: false`**, the pair draw mode turns off together — and a click the clamp's mask rejects but its **bbox** contains drops the clamp rather than refusing. The bbox is load-bearing in both directions: outside it the click is on blank page, and refusing there is the one thing the clamp was really doing, so that case is now refused in its own words.
  ...
  **Quality is a first-class output.** `traceFloorplanBoundaryCore` returns `quality: {confidence, warnings[], usedFallback, source, …}`; the worker forwards a whitelist of debug fields (it must never blanket-null them again); traces carry their quality into the store; `App.jsx` reports a doubtful trace as doubtful. `src/utils/boundaryQuality.js` decides the wording.
  ...
  Constraints flow the other way too: `options.constraints` carries `rooms[]` (known-inside rectangles) and `interiorPoints` (parsed dimension labels) from the store, geometry that excludes them is scored down, and a winner that still excludes one is re-searched (`remediate.js`). **Excluding one is subtracted outside the weighted mean**, beside `annex` and `incomplete` — a contradiction, not a lower mark. As a reward term alone it was worth 22/122 of the score times one label in nine, which `economy` buys back on a tidy closing radius: on ExampleFloorplan9, whose bottom-right corner is drawn open, the only candidate enclosing every label lost by 0.005 to one that left a whole storage room outside.

  **The benchmark runs each fixture twice** — once bare and once with the truth file's room clicks as constraints — because that second run is the path the app actually takes after a scan, and it is the only one that exercises remediation. A truth file may set `boundary.bare` to override thresholds for the first run only: on some plans tracing before anything has been read off the page is a strictly weaker position rather than a bug, and lowering the bar for both runs would stop the constrained run — the one the app takes — from being scored at all.
