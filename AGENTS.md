...
`App.jsx` still owns every workflow decision. It builds the `<Canvas>` element once (`canvasElement`) and hands it to whichever shell renders: the desktop shell — one full-width band (the menu titles and the command bar share it) over a row of dock, plan column and tool rail, with the tab strip and status bar stacked inside the plan's column — or `<MobileChrome>` (`src/components/mobile/`), which is a top bar, the plan, one thumb-height bar, and three sheets over a shared `BottomSheet`. Do not fork behaviour across the two — the mobile measurement sheet renders the *same* `MeasurementDock` with `mobile`, re-sized from outside by the `.touch-dense` scope in `index.css`, and the tool sheet reads the same `TOOL_GROUPS` (`components/toolCatalog.js`) the desktop rail does.
...
`StatusBar` is a 26 px band of the plan's own column, under the tab strip and over the canvas, inset between the measurement dock and the tool rail. Only the hint cell truncates and nothing scrolls. That one cell has four claimants in order — processing, a `statusFlash`, the tool the pointer is resting on, then the running mode's instruction — and the hover text renders outside the `role="status"` live region, which is `aria-atomic` and would otherwise re-announce on every one of twelve rail buttons. The band **is** the context bar: a running tool's name, brush, corner count and Cancel/Done live here, the standing cells (scale, zoom, a healthy draft) stand down to make room, and the band tints accent. The tool rail is a single 48 px width: hover or focus writes `{name, detail, digit}` into `workspaceStore.toolHint`, which replaced the `showLabels` preference and both of its toggles. The menu bar carries three titles — Settings and Help were folded into File. See CLAUDE.md for the full version of all of this; this file is a mirror and drifts.

**The tool digits run 1–9 straight down `TOOL_GROUPS`.** Both `toolCatalog.js` and `useKeyboardShortcuts.js` claimed to match and did not — the rail read 7, 4, 8, 9, 1, 3, 2, 5, 6 top to bottom, because digits were handed out in the order the tools were built and the rail was regrouped around them later. Nothing derives a digit from an index (a mapping that moves with app state is the thing being avoided), so renumbering means editing both lists together, plus the four places that print a digit: the `keys` on the two Trace menu items, the Paint-outline tooltip in `CommandBar`, the `1 – 9` row in `HelpModal`, and the badge in the corner of every rail button.
